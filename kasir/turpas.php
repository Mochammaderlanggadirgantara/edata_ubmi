...
$allowed_roles = ['pengawas', 'pimpinan', 'kepala mantri', 'kasir'];
...
$sqlUser = "
    SELECT t.id_user, t.jabatan, t.nama_user, t.id_cabang, c.nama_cabang
    FROM tuser t
    JOIN cabang c ON t.id_cabang = c.id_cabang
    WHERE t.id_user = '$id_user'
";
$qUser = mysqli_query($conn, $sqlUser);
...
// AMBIL PERIODE BULAN
// =======================================================
$pendapatan_bulan = $_POST['pendapatan_bulan'] ?? date('Y-m'); // default bulan sekarang

if (!empty($pendapatan_bulan) && preg_match('/^\d{4}-\d{2}$/', $pendapatan_bulan)) {
    list($tahun, $bulan) = explode('-', $pendapatan_bulan);
} else {
    $tahun = date('Y');
    $bulan = date('m');
}
...
$periode_akhir = date("Y-m-t", strtotime($periode_awal)); // akhir bulan
    ...
    WHERE jabatan != 'mantri'
      ...
      AND (
         ...
         OR (
            status = 'tidak aktif'
            AND (
                tgl_nonaktif IS NULL 
                OR tgl_nonaktif = '0000-00-00'
                OR tgl_nonaktif > '$periode_akhir'
            )
         )
      )
";
$resultNonMantri = mysqli_query($conn, $sqlNonMantri);
...
";
$resultMantri = mysqli_query($conn, $sqlMantri);

// =======================================================
// CEK HASIL QUERY (opsional)
// =======================================================
if (!$resultNonMantri) {
    die("Query Non-Mantri gagal: " . mysqli_error($conn));
}
if (!$resultMantri) {
    die("Query Mantri gagal: " . mysqli_error($conn));
}
?>
...
if ($resultNonMantri && mysqli_num_rows($resultNonMantri) > 0) {
    while ($row = mysqli_fetch_assoc($resultNonMantri)) : ?>
        <tr class="baris" data-id-user="<?= $row['id_user'] ?>">
            <td><?= $no++ ?></td>
            <td>
...
<!-- === Mantri === -->
<?php
if ($resultMantri && mysqli_num_rows($resultMantri) > 0) {
    while ($rowMantri = mysqli_fetch_assoc($resultMantri)) : ?>
        <tr class="barisMantri" data-id-user="<?= $rowMantri['id_user'] ?>">
            <td><?= $no++ ?></td>
            <td>
...
<tr class="fw-bold" id="baris-total-mantri">
    ...
    <td>-</td>
    <td>-</td>
    <td><span class="total-mantri-gaji"></span><br><small class="text gabungan-gaji"></small></td>
    <td><span class="total-mantri-rumus"></span><br><small class="text gabungan-rumus"></small></td>
    <td><span class="total-mantri-cm"></span><br><small class="text gabungan-cm"></small></td>
    <td><span class="total-mantri-mb"></span><br><small class="text gabungan-mb"></small></td>
    <td><span class="total-mantri-ml"></span><br><small class="text gabungan-ml"></small></td>
    <td><span class="total-mantri-do"></span><br><small class="text gabungan-do"></small></td>
    <td><span class="total-mantri-goro"></span><br><small class="text gabungan-goro"></small></td>
    <td><span class="total-mantri-um"></span><br><small class="text gabungan-um"></small></td>
    <td><span class="total-mantri-jumlah_pendapatan"></span><br><small class="text gabungan-jumlah_pendapatan"></small></td>
    <td class="border-split"><span class="total-mantri-bon_prive"></span><br><small class="text gabungan-bon_prive"></small></td>
    <td><span class="total-mantri-beban"></span><br><small class="text gabungan-beban"></small></td>
    <td><span class="total-mantri-wajib"></span><br><small class="text gabungan-wajib"></small></td>
    <td><span class="total-mantri-sukarela"></span><br><small class="text gabungan-sukarela"></small></td>
    <td><span class="total-mantri-absensi"></span><br><small class="text gabungan-absensi"></small></td>
    <td><span class="total-mantri-lain_lain"></span><br><small class="text gabungan-lain_lain"></small></td>
    <td><span class="total-mantri-jumlah_potongan"></span><br><small class="text gabungan-jumlah_potongan"></small></td>
    <td class="border-split"><span class="total-mantri-pendapatan_bersih"></span><br><small class="text gabungan-pendapatan_bersih"></small></td>
</tr>
</tbody>
...
<!-- Overlay Loading -->
<div id="overlay-loading" class="d-none position-fixed top-0 start-0 w-100 h-100 bg-white bg-opacity-75 d-flex flex-column justify-content-center align-items-center" style="backdrop-filter: blur(4px); z-index: 9999;">
    <div class="spinner-border text-primary" role="status" style="width: 4rem; height: 4rem;">
        <span class="visually-hidden">Loading...</span>
    </div>
    <div class="mt-3 fw-semibold text-primary">Memuat data...</div>
</div>
...
<script>
    ...
    function formatNumberInput(input) {
        if (input.classList.contains('persen')) {
            // Hanya izinkan angka & koma (tidak langsung ubah ke titik)
            let val = input.value.replace(/[^0-9,]/g, '');
            const parts = val.split(',');
            if (parts.length > 2) {
                val = parts[0] + ',' + parts[1]; // cuma satu koma
            }
            input.value = val;
            return;
        }

        // Format ribuan untuk angka biasa
    ...
    $('#formTurpas').on('submit', function(e) {
        ...
        $('tr.baris, tr.barisMantri').each(function() {
            ...
            detailData.push({
                ...
                persen: $(this).find('input.persen').val(),
        ...
        // 🔹 Tambahkan console untuk debug
        console.log("Data yang dikirim ke PHP:");
        console.log({
...
<script>
    document.getElementById('pendapatan_bulan').addEventListener('change', function() {
        ...
        // 🔹 Panggil AJAX filter
        fetch(`kasir/get_turpas.php?pendapatan_bulan=${value}&gaji_bulan=${yyyy}-${mm}-01`)
            .then(res => res.json())
            .then(data => {
                if (data.status === 'success') {
                    // Isi setoran
                    document.getElementById('setoran').value = formatNumber(data.master.setoran);

                    // Isi detail sesuai id_user
                    data.detail.forEach(d => {
                        const row = document.querySelector(`tr[data-id-user="${d.id_user}"]`);
                        if (row) {
                            row.querySelector('.drop').value = formatNumber(d.drop_val);
                            row.querySelector('.storting').value = formatNumber(d.storting);
                            row.querySelector('.persen').value = d.persen;
                            row.querySelector('.gaji').value = formatNumber(d.gaji);
                            row.querySelector('.cm').value = formatNumber(d.cm);
                            row.querySelector('.mb').value = formatNumber(d.mb);
                            row.querySelector('.ml').value = formatNumber(d.ml);
                            row.querySelector('.goro').value = formatNumber(d.goro);
                            row.querySelector('.um').value = formatNumber(d.um);
                            row.querySelector('.bon_prive').value = formatNumber(d.bon_prive);
                            row.querySelector('.beban').value = formatNumber(d.beban);
                            row.querySelector('.wajib').value = formatNumber(d.wajib);
                            row.querySelector('.sukarela').value = formatNumber(d.sukarela);
                            row.querySelector('.absensi').value = formatNumber(d.absensi);
                            row.querySelector('.lain_lain').value = formatNumber(d.lain_lain);
                        }
                    });

                    updateDropStortingNonMantri();
                    setupBaris();

                    // 🔹 Paksa hitung index & rumus untuk semua baris
                    document.querySelectorAll('tr.baris, tr.barisMantri').forEach(row => {
                        const drop = row.querySelector('.drop');
                        if (drop) {
                    ...
                    // Kosongkan semua input kalau tidak ada data
                    document.getElementById('setoran').value = '';
                    document.querySelectorAll('input.form-control').forEach(el => el.value = '');

                    // Tetap set pendapatan_bulan & gaji_bulan sesuai pilihan
                    document.getElementById('pendapatan_bulan').value = `${year}-${String(month).padStart(2, '0')}`;
                    document.getElementById('gaji_bulan').value = `${yyyy}-${mm}-01`;
                    updateTotalPerKolom();
                    updateTotalPerKolomMantri();
                }
