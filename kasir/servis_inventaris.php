...
// query data servis
$query  = "SELECT * FROM riwayat_servis WHERE id_cabang = '$id_cabang'";
    ...
    <div class="container-fluid">
        ...
        <div class="main-content-container overflow-hidden">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
                <h3 class="mb-0">Data Servis Inventaris</h3>
                ...
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                    <ol class="breadcrumb align-items-center mb-0 lh-1">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">
                            <span class="fw-medium">Data Servis Inventaris</span>
                        </li>
                ...
                <div class="card-body p-4">
                    <ul class="nav nav-tabs border-0 gap-3 mb-lg-4 mb-3 seller-tabs" id="myTab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link border-0 bg-body-bg text-secondary fw-medium fs-16 rounded-3 py- px-md-5 px-4 active" id="all-tab" data-bs-toggle="tab" data-bs-target="#all-tab-pane" type="button" role="tab" aria-controls="all-tab-pane" aria-selected="true">
                                <span class="d-inline-block py-1">Pilih cabang</span>
                            </button>
                        </li>
                    ...
                    <div class="button-container d-flex justify-content-between flex-wrap gap-3 mb-4">
                        ...
                        <?php if ($_SESSION['jabatan'] === 'kasir') : ?>
                            <button onclick="window.location.href='create_inventaris.php'" class="btn btn-outline-primary fs-16 py-2 px-4 ms-auto">Tambah Data</button>
                        <?php endif; ?>
                    </div>
                                    ...
                                    <?php if ($result->num_rows > 0): ?>
                                        ...
                                        while ($row = $result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><?= date('d F Y', strtotime($row['tanggal_servis'])) ?></td>
                                                <td><?= htmlspecialchars($row['nama_part_jasa']) ?></td>
                                                <td>Rp <?= number_format($row['sub_total'], 0, ',', '.') ?></td>
                                                <td><?= date('d F Y', strtotime($row['tanggal_servis_selanjutnya'])) ?></td>
                                                <td>
                                                    <a href="edit_servis_inventaris.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Edit</a>
                                                    <a href="delete_servis_inventaris.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</a>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else: ?>
