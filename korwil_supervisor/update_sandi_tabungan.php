...
$id = $_GET['id'];
$sandi = $conn->query("SELECT * FROM sandi WHERE id_sandi = $id")->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE sandi SET kata_sandi=?, id_cabang=? WHERE id_sandi=?");
    $stmt->bind_param("sii", $kata_sandi, $id_cabang, $id);
    $stmt->execute();
...
<!-- End Preloader Area -->
    ...
    ?>
    <h3>Edit Kata Sandi</h3>
    <form method="POST" class="mt-4">
        <div class="mb-3">
            <label for="kata_sandi" class="form-label">Kata Sandi</label>
            <input type="password" name="kata_sandi" id="kata_sandi" value="<?= $sandi['kata_sandi'] ?>" class="form-control" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_cabang" id="id_cabang" class="form-select" required>
                <?php while ($row = $cabang->fetch_assoc()): ?>
                    <option value="<?= $row['id_cabang'] ?>" <?= $sandi['id_cabang'] == $row['id_cabang'] ? 'selected' : '' ?>>
                        <?= $row['nama_cabang'] ?>
                    </option>
                <?php endwhile; ?>
            </select>
