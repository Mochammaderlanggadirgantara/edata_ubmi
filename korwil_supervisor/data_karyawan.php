...
// Ambil data user
$query = "SELECT t.*, c.nama_cabang FROM TUser t LEFT JOIN cabang c ON t.id_cabang = c.id_cabang ORDER BY t.tgl_masuk DESC";
$query = "
SELECT u.*, c.nama_cabang, k.nama_kelompok 
FROM tuser u
        ...
        <div class="card bg-white border-0 rounded-3 mb-4">
            <div class="card-body p-4">
                <div class="button-container d-flex justify-content-between flex-wrap gap-3 mb-4">
                    ...
                    <button onclick="window.location.href='add_user.php'" class="btn btn-outline-primary fs-16 py-2 px-4 ms-auto">Tambah Data</button>
                    ...
                    <div class="default-table-area all-products">
                        <table class="table align-middle w-100" id="myTables">
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo '<tr>
                    <td>' . $no . '</td>
                    <td data-label="nama_user">
                        <div class="d-flex align-items-center">
                            <img src="/edata_ubmi/assets/images/user-6.jpg" alt="user">
                            <div class="ms-2 ps-1">
                                <h6 class="fw-medium fs-14 mb-0">' . $row['nama_user'] . '</h6>
                            </div>
                        </div>
                    </td>
                    <td data-label="Jabatan" class="text-secondary">' . $row['jabatan'] . '</td>
                     <td data-label="Jabatan" class="text-secondary">' . $row['nama_cabang'] . '</td>
                    <td data-label="tgl_masuk" class="text-secondary">' . $row['tgl_masuk'] . '</td>
                    <td data-label="username" class="text-secondary">' . $row['username'] . '</td>
                    <td data-label="Kelompok" class="text-secondary">' . ($row['nama_kelompok'] ?? '-') . '</td>
                    <td data-label="Kelompok" class="text-secondary">' . ($row['status'] ?? '-') . '</td>
                    <td data-label="Action">
                        <div class="d-flex align-items-center gap-1 justify-content-center">
                            <a href="edit_user.php?id=' . $row['id_user'] . '" class="ps-0 border-0 bg-transparent lh-1" title="Edit">
                                <i class="material-symbols-outlined fs-16 text-body">edit</i>
                            </a>
                            <a href="delete_user.php?id=' . $row['id_user'] . '" class="ps-0 border-0 bg-transparent lh-1" title="Delete" onclick="return confirm(\'Yakin ingin menghapus data ini?\');">
